...
# Set the workspace
arcpy.env.workspace = "F:\\independent_study\\topo_efficiency"
...
csv_output_file = "F:\\independent_study\\topo_efficiency\\topo_efficiency.csv"
...
for recordnumber in range(0,1):
	...
	arcpy.CopyFeatures_management("bb_r%r" % recordnumber, "F:\\independent_study\\topo_efficiency\\bboard_points\\bb_r%r" % recordnumber)

	# Create a 1000ft buffer around current billboard
	arcpy.Buffer_analysis("F:\\independent_study\\topo_efficiency\\bboard_points\\bb_r%r.shp" % recordnumber, "F:\\independent_study\\topo_efficiency\\bboard_buffer\\bb_r%rbuf" % recordnumber, buffersize)

	# Clip the subset DEM to the individual billboard buffer
	arcpy.Clip_management("F:\\independent_study\\topo_efficiency\\fullclip_test", "#", "F:\\independent_study\\topo_efficiency\\bboard_clip\\bb_r%rclip" % recordnumber, "F:\\independent_study\\topo_efficiency\\bboard_buffer\\bb_r%rbuf.shp" % recordnumber, "#", "ClippingGeometry")

	# Retrieves 3D analyst license, then creates a viewshed within individual DEM, then returns license to license manager
	arcpy.CheckOutExtension("3D")
	arcpy.Viewshed_3d("F:\\independent_study\\topo_efficiency\\bboard_clip\\bb_r%rclip" % recordnumber, "F:\\independent_study\\topo_efficiency\\bboard_points\\bb_r%r.shp" % recordnumber, "F:\\independent_study\\topo_efficiency\\bboard_viewshed\\bb_r%rvshd" % recordnumber)
	arcpy.CheckInExtension("3D")

	# Pulls number of visible and not-visible pixels out of the table. Stores in variables vispix and nvispix, respectively.
	with arcpy.da.SearchCursor("F:\\independent_study\\topo_efficiency\\bboard_viewshed\\bb_r%rvshd" % recordnumber, ("COUNT"), ' "Rowid" = 0') as cursor:
		for row in cursor:
			nvispix = row[0]
		
	with arcpy.da.SearchCursor("F:\\independent_study\\topo_efficiency\\bboard_viewshed\\bb_r%rvshd" % recordnumber, ("COUNT"), ' "Rowid" = 1') as cursor:
		for row in cursor:
			vispix = row[0]
